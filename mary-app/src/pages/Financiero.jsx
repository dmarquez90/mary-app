...
export default function Financiero() {
  ...
  const TABS = [t('fin_tab_direct'), t('fin_tab_payroll'), t('fin_tab_sub'), t('fin_tab_equipment'), t('fin_tab_indirect')]
  const tabLabels = [t('fin_tab_direct'), t('fin_tab_payroll'), t('fin_tab_sub'), t('fin_tab_equipment'), t('fin_tab_indirect')]
    ...
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
        ...
        <div className="flex items-center gap-3">
          ...
          {proyId && !closed && <PrimaryBtn onClick={openDrawer}>+ {tabLabels[tab]}</PrimaryBtn>}
        </div>
      </div>
        ...
        <>
          ...
          {tabEmpty.length === 0 ? (
            <EmptyState icon={Icons.financial} title={t('fin_empty')}
              action={closed ? undefined : `+ ${tabLabels[tab]}`} onAction={openDrawer} />
          ) : (
            <div className="bg-white rounded-xl border border-gray-100 overflow-x-auto">
              {tab === 0 && <table className="w-full"><thead><tr className="bg-gray-50 border-b border-gray-100">
                {[t('fin_col_date'),t('fin_col_type'),t('fin_col_desc'),t('fin_col_amount'),t('fin_col_doc'),t('fin_col_activity')].map(h=><th key={h} className="px-4 py-3 text-left text-xs text-gray-500">{h}</th>)}
              </tr></thead><tbody>
                {directs.map(c => {
                  ...
                  return <tr key={c.id} className="border-b border-gray-50 hover:bg-gray-50/50">
                    ...
                    <td className="px-4 py-3"><span className={`text-xs px-2 py-0.5 rounded-full font-medium ${c.tipo==='caja_chica'?'bg-amber-100 text-amber-700':'bg-blue-100 text-blue-700'}`}>{c.tipo==='caja_chica'?t('fin_petty'):t('fin_invoice')}</span></td>
                ...
                <tr className="bg-gray-50"><td colSpan={3} className="px-4 py-2 text-right text-xs font-semibold text-gray-500">{t('lbl_total')}</td><td className="px-4 py-2 text-sm font-mono font-bold" style={{color:'#1D9E75'}}>{fmt(totalDir,moneda)}</td><td colSpan={2}/></tr>
              </tbody></table>}

              {tab === 1 && <table className="w-full"><thead><tr className="bg-gray-50 border-b border-gray-100">
                {[t('fin_col_worker'),t('fin_col_position'),t('fin_col_period'),t('fin_col_base'),t('fin_col_deductions'),t('fin_col_net')].map(h=><th key={h} className="px-4 py-3 text-left text-xs text-gray-500">{h}</th>)}
              </tr></thead><tbody>
                {noms.map(n => {
                ...
                <tr className="bg-gray-50"><td colSpan={5} className="px-4 py-2 text-right text-xs font-semibold text-gray-500">{t('fin_col_net')}</td><td className="px-4 py-2 text-sm font-mono font-bold" style={{color:'#1D9E75'}}>{fmt(totalNom,moneda)}</td></tr>
              </tbody></table>}

              {tab === 2 && <table className="w-full"><thead><tr className="bg-gray-50 border-b border-gray-100">
                {[t('fin_col_subcontractor'),t('fin_col_work'),t('fin_col_contract'),t('fin_col_progress'),t('fin_col_paid'),t('lbl_status')].map(h=><th key={h} className="px-4 py-3 text-left text-xs text-gray-500">{h}</th>)}
              </tr></thead><tbody>
                {subs.map(sc => (
                ...
                <tr className="bg-gray-50"><td colSpan={4} className="px-4 py-2 text-right text-xs font-semibold text-gray-500">{t('fin_col_paid')}</td><td className="px-4 py-2 text-sm font-mono font-bold" style={{color:'#1D9E75'}}>{fmt(totalSub,moneda)}</td><td/></tr>
              </tbody></table>}

              {tab === 3 && <table className="w-full"><thead><tr className="bg-gray-50 border-b border-gray-100">
                {[t('fin_col_equipment'),t('fin_col_type'),t('fin_col_rate'),t('fin_col_days'),t('fin_col_total')].map(h=><th key={h} className="px-4 py-3 text-left text-xs text-gray-500">{h}</th>)}
              </tr></thead><tbody>
                {eqs.map(e => (
                ...
                <tr className="bg-gray-50"><td colSpan={4} className="px-4 py-2 text-right text-xs font-semibold text-gray-500">{t('lbl_total')}</td><td className="px-4 py-2 text-sm font-mono font-bold" style={{color:'#1D9E75'}}>{fmt(totalEq,moneda)}</td></tr>
              </tbody></table>}

              {tab === 4 && <table className="w-full"><thead><tr className="bg-gray-50 border-b border-gray-100">
                {[t('fin_col_date'),t('fin_col_category'),t('fin_col_desc'),t('fin_col_amount')].map(h=><th key={h} className="px-4 py-3 text-left text-xs text-gray-500">{h}</th>)}
              </tr></thead><tbody>
                {inds.map(c => (
                ...
                <tr className="bg-gray-50"><td colSpan={3} className="px-4 py-2 text-right text-xs font-semibold text-gray-500">{t('lbl_total')}</td><td className="px-4 py-2 text-sm font-mono font-bold" style={{color:'#1D9E75'}}>{fmt(totalInd,moneda)}</td></tr>
              </tbody></table>}
            </div>
          )}
        </>
      ...
      <Drawer open={drawer} onClose={() => setDrawer(false)} title={`+ ${tabLabels[tab]}`} width={400}>
        {tab === 0 && <>
          <Field label={t('fin_form_type')}><select className={selectCls} value={form.tipo||'factura_obra'} onChange={set('tipo')}>
            <option value="factura_obra">{t('fin_invoice')}</option><option value="caja_chica">{t('fin_petty')}</option>
          </select></Field>
          ...
          <Field label={t('fin_form_activity')}><select className={selectCls} value={form.actividad_id||''} onChange={set('actividad_id')}>
            <option value="">{t('lbl_select')}</option>{acts.map(a=><option key={a.id} value={a.id}>{a.code} — {a.descripcion}</option>)}
          </select></Field>
        ...
        {tab === 3 && <>
          ...
          <Field label={t('fin_form_eq_type')}><select className={selectCls} value={form.tipo||'alquiler'} onChange={set('tipo')}>
            <option value="alquiler">Alquiler</option><option value="propio">Propio</option>
          </select></Field>
          <div className="grid grid-cols-2 gap-3">
            <Field label={t('fin_form_eq_rate')}><input type="number" className={inputCls} value={form.tarifa_diaria||''} onChange={set('tarifa_diaria')} placeholder="0.00" min="0" step="0.01"/></Field>
            <Field label={t('fin_form_eq_days')}><input type="number" className={inputCls} value={form.dias_uso||''} onChange={set('dias_uso')} placeholder="0" min="0"/></Field>
          </div>
          <Field label={t('fin_form_eq_total')} required><input type="number" className={inputCls} value={form.costo_total||((parseFloat(form.tarifa_diaria)||0)*(parseFloat(form.dias_uso)||0)||'')} onChange={set('costo_total')} placeholder="0.00" min="0" step="0.01"/></Field>
        </>}
        {tab === 4 && <>
