  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-5">
        ...
        {tab === 0 && (
          <PrimaryBtn onClick={() => {
            setForm({ proyecto_id:'', actividad_id:'', justificacion:'' })
            setSolItems([{ material_id:'', cantidad:'', unidad:'und' }])
            setDrawer('sol')
          }}>{t('comp_new_sol')}</PrimaryBtn>
        )}
        {tab === 1 && (
          <PrimaryBtn onClick={() => { setForm({ solicitud_id:'', proveedor:'' }); setDrawer('oc') }}>
            {t('comp_new_oc')}
          </PrimaryBtn>
        )}
      </div>
      ...
      {tab === 0 && (
        solicitudes.length === 0 ? (
          <EmptyState icon={Icons.purchases} title={t('comp_empty_sol')}
            action={t('comp_new_sol')} onAction={() => {
              setForm({ proyecto_id:'', actividad_id:'', justificacion:'' })
              setSolItems([{ material_id:'', cantidad:'', unidad:'und' }])
              setDrawer('sol')
            }} />
                    ...
                    <tr key={sol.id} className="border-b border-gray-50 hover:bg-gray-50/50">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex gap-1 flex-wrap">
                          ...
                          {sol.estado === 'aprobada' && (
                            <TBtn onClick={() => { setForm({ solicitud_id: sol.id, proveedor:'' }); setDrawer('oc') }}>
                              {t('comp_generate_oc')}
                            </TBtn>
                          )}
                        </div>
      ...
      <Drawer open={drawer==='detalle'} onClose={() => setDrawer(null)} title={t('comp_detail_sol')} width={400}>
        {detSol && <>
          ...
          {detSol.justificacion && (
            <div>
              <p className="text-xs text-gray-400">{t('comp_form_justification')}</p>
              <p className="text-sm text-gray-700">{detSol.justificacion}</p>
            </div>
          )}
          <div>
