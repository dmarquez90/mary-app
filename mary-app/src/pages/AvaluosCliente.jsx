...
export default function AvaluosCliente() {
  ...
  // Presupuesto original + OC aprobadas
  ...
  // Número del próximo avalúo
  const numAvaluo = avs.length + 1

  // Calcular acumulado previo para una actividad
  ...
  // Cantidad total de una actividad (incluyendo OC aprobadas)
  const cantidadTotal = (act) => {
    const base = parseFloat(act.cantidad || 0)
    const ocExtra = ocItemsAprobados
      .filter(i => i.actividad_id === act.id)
      .reduce((s,i) => s + parseFloat(i.diferencia || 0), 0)
    return base + ocExtra
  }

  // P.U. de una actividad
  ...
  const openNuevo = () => {
    ...
    // Inicializar items desde actividades + OC nuevas
    const itemsBase = actividades.map(act => {
      const ant    = acumuladoPrevio(act.id)
      const total  = cantidadTotal(act)
      const pu     = precioUnitario(act)
      return {
        actividad_id:      act.id,
    ...
    // Agregar actividades nuevas de OC (tipo 'nueva')
    const itemsNuevos = ocItemsAprobados
  ...
  const setItemPeriodo = (idx, v) => {
    ...
    setAvItems(prev => prev.map((it,i) => {
      ...
      return {
        ...it,
        cantidad_periodo:  v,
        cantidad_acumulada: acum,
        cantidad_saldo:     saldo,
        pct_fisico:         pct,
        monto_periodo:      mp,
        monto_anterior:     ma,
        monto_acumulado:    ma + mp,
        monto_saldo:        saldo * parseFloat(it.precio_unitario||0),
      }
    }))
  }
  ...
  const excede = (it) => parseFloat(it.cantidad_acumulada||0) > parseFloat(it.cantidad_total||0)

  // Items a mostrar en el formulario
  const itemsVisibles = mostrarTodas ? avItems : avItems.filter(it => parseFloat(it.cantidad_periodo||0) > 0 || it.cantidad_total > 0)
    ...
    return (
      <div className="p-6 max-w-5xl mx-auto">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h1 className="text-xl font-semibold text-gray-800">
              {isEs ? `Avalúo #${numAvaluo}` : `Valuation #${numAvaluo}`} — {proy?.project_code}
            </h1>
        ...
        <div className="bg-white border border-gray-100 rounded-xl p-5 flex flex-col gap-4">
          ...
          {/* Presupuesto efectivo info */}
          {totalOCAprobadas !== 0 && (
            <div className="grid grid-cols-3 gap-3 p-3 bg-blue-50 rounded-xl border border-blue-100 text-xs">
              <div>
                <p className="text-blue-500">{isEs ? 'Presupuesto original' : 'Original budget'}</p>
                <p className="font-bold font-mono text-blue-700">{fmt(presupuestoOriginal, moneda)}</p>
              </div>
              <div>
                <p className="text-blue-500">{isEs ? 'OC aprobadas' : 'Approved COs'}</p>
                <p className="font-bold font-mono text-blue-700">{totalOCAprobadas >= 0 ? '+' : ''}{fmt(totalOCAprobadas, moneda)}</p>
              </div>
              <div>
                <p className="text-blue-500">{isEs ? 'Presupuesto efectivo' : 'Effective budget'}</p>
                <p className="font-bold font-mono text-blue-800">{fmt(presupuestoEfectivo, moneda)}</p>
              </div>
            </div>
          )}
          ...
          <div className="flex items-center justify-between">
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">
              {isEs ? 'Avance físico por actividad' : 'Physical progress by activity'}
            </p>
              ...
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100">
                  {[
                    ...
                    isEs?'Unidad':'Unit',
                    isEs?'Cant. Total':'Total Qty',
                    isEs?'P.U.':'Unit Price',
                    isEs?'Monto Contrato':'Contract Amount',
                    isEs?'Ant.':'Prev.',
                    isEs?'Este Período *':'This Period *',
                    isEs?'Acumulado':'Accumulated',
                    isEs?'Saldo':'Balance',
                    isEs?'% Fís.':'% Phys.',
                    isEs?'Monto Período':'Period Amount',
                  ...
                  return (
                    <tr key={idx} className={`border-b border-gray-50 ${sobrepa ? 'bg-red-50/30' : ''}`}>
                      <td className="px-2 py-2 text-gray-700">
                        {it.descripcion}
                        {it.es_oc && <span className="ml-1 text-xs px-1 py-0.5 rounded bg-amber-100 text-amber-700">OC</span>}
                      </td>
                      ...
                      <td className="px-2 py-2">
                        <input type="number"
                          className={`w-24 border rounded-lg px-2 py-1 text-xs focus:outline-none ${
                            sobrepa ? 'border-red-400 bg-red-50' : 'border-gray-200 bg-white focus:border-[#1B3A6B]'}`}
                          value={it.cantidad_periodo}
                          placeholder="0"
                          min="0" step="0.01"
                          onChange={e => setItemPeriodo(idx, e.target.value)} />
                        {sobrepa && <p className="text-xs text-red-500 mt-0.5">⚠ {isEs?'Excede':'Exceeds'}</p>}
                      </td>
                      <td className="px-2 py-2 font-mono" style={{color: sobrepa?'#ef4444':'#1D9E75'}}>
                        {fmtNum(acum)}
                      </td>
                        ...
                        <div className="flex items-center gap-1.5">
                          <div className="w-16 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                            <div className="h-full rounded-full" style={{
                              ...
                              background: sobrepa ? '#ef4444' : pct >= 100 ? '#1D9E75' : BRAND,
                            }} />
                          </div>
                          <span className="font-mono" style={{color: sobrepa?'#ef4444':BRAND}}>
                            {pct.toFixed(1)}%
                          </span>
                        </div>
                      </td>
                      <td className="px-2 py-2 font-mono font-medium" style={{color:'#1D9E75'}}>
                        {mp > 0 ? fmt(mp, moneda) : '—'}
                      </td>
                    </tr>
                  )
            ...
            <div className="bg-blue-50 rounded-xl p-3 border border-blue-100">
              <div className="flex items-center justify-between mb-1">
                ...
                <span className="text-xs font-mono text-blue-700">
                  {fmt(totalCobrado + avSubtotal, moneda)} / {fmt(presupuestoEfectivo, moneda)}
                </span>
              </div>
              <div className="w-full h-2 bg-blue-100 rounded-full overflow-hidden">
                <div className="h-full rounded-full bg-blue-600"
                  style={{width:`${Math.min(100,((totalCobrado+avSubtotal)/presupuestoEfectivo)*100)}%`}} />
              </div>
            </div>
          )}
          ...
          <div className="border-t border-gray-100 pt-3">
            {[
              [isEs?'Subtotal período':'Period subtotal', fmt(avSubtotal, moneda)],
            ].map(([k,v]) => (
          ...
          <div className="flex gap-2 pt-2">
            ...
            <PrimaryBtn onClick={saveAvaluo}
              disabled={avSubtotal === 0}
              className="flex-1">
      ...
      <div className="p-6 max-w-5xl mx-auto">
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-semibold text-gray-800">
                {isEs ? `Avalúo #${avDetalle.numero}` : `Valuation #${avDetalle.numero}`}
              </h1>
          ...
          <div className="flex items-center gap-2">
            ...
            {puedeEditar && avDetalle.estado === 'borrador' && (
              <button onClick={() => dispatch({ type:'UPD_AVALUO_CLIENTE_ESTADO', payload:{ id:avDetalle.id, estado:'presentado' } })}
                className="text-xs px-3 py-1.5 rounded-lg text-white font-medium"
                style={{ background: '#185FA5' }}>
                {isEs ? 'Presentar al cliente' : 'Submit to client'}
              </button>
            )}
            {puedeEditar && avDetalle.estado === 'presentado' && (
              <button onClick={() => dispatch({ type:'UPD_AVALUO_CLIENTE_ESTADO', payload:{ id:avDetalle.id, estado:'aprobado' } })}
                className="text-xs px-3 py-1.5 rounded-lg text-white font-medium bg-green-600">
                {isEs ? 'Marcar aprobado' : 'Mark approved'}
              </button>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="bg-white border border-gray-100 rounded-xl p-5">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
              {[
                [isEs?'Período':'Period', avDetalle.periodo_inicio && avDetalle.periodo_fin
                  ? `${avDetalle.periodo_inicio} → ${avDetalle.periodo_fin}` : '—'],
          ...
          <div className="bg-white border border-gray-100 rounded-xl overflow-hidden">
            <div className="bg-gray-50 px-4 py-2.5 border-b border-gray-100">
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">
                {isEs ? 'Detalle de avance' : 'Progress detail'}
              </p>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                      ...
                      <td className="px-3 py-2">
                        <div className="flex items-center gap-1.5">
                          <div className="w-12 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                            <div className="h-full rounded-full" style={{
                              width:`${Math.min(100,parseFloat(it.pct_fisico||0))}%`,
                              background: BRAND,
                            }} />
                          </div>
                          <span className="font-mono" style={{color:BRAND}}>
                            {parseFloat(it.pct_fisico||0).toFixed(1)}%
                          </span>
                        </div>
                      </td>
            ...
            <div className="px-4 py-4 border-t border-gray-100 flex flex-col gap-1.5">
              {[
                [isEs?'Subtotal período':'Period subtotal', fmt(avSubtotal2, moneda)],
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-semibold text-gray-800">
            {isEs ? 'Avalúos al Cliente' : 'Client Valuations'}
          </h1>
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-5">
            {[
              ...
              { label: isEs?'% Ejecución financiera':'% Financial execution',value: `${pctEjecucion.toFixed(1)}%`, color: BRAND },
            ].map(k => (
          ...
          {avs.length === 0 ? (
            <div className="bg-white border border-gray-100 rounded-xl py-16">
              <EmptyState icon={Icons.budget}
                title={isEs ? 'No hay avalúos registrados' : 'No valuations registered'}
                subtitle={isEs ? 'Crea el primer avalúo para cobrar al cliente' : 'Create the first valuation to bill the client'}
                action={puedeEditar ? (isEs ? '+ Nuevo Avalúo' : '+ New Valuation') : null}
