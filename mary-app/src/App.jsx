    ...
    <div className="flex h-screen overflow-hidden bg-gray-50">
      <aside className={`flex flex-col bg-gray-900 transition-all duration-200 ${sideOpen ? 'w-52' : 'w-14'} flex-shrink-0`}>
        <div className="flex items-center gap-3 px-4 py-5 border-b border-gray-800">
          ...
          {sideOpen && (
            <div>
              ...
              <p className="text-gray-500 text-xs mt-0.5">ERP {lang === 'es' ? 'Construcción' : 'Construction'}</p>
            </div>
          )}
        </div>
        ...
        <nav className="flex-1 py-4 flex flex-col gap-0.5 px-2">
          {NAV.map(item => (
            <button
              ...
              title={!sideOpen ? (lang === 'es' ? item.labelEs : item.labelEn) : ''}
            >
              <span className="w-4 h-4 flex-shrink-0">{Icons[item.icon]}</span>
              {sideOpen && <span className="text-sm font-medium truncate">{lang === 'es' ? item.labelEs : item.labelEn}</span>}
            </button>
          ))}
        </nav>
        ...
        <div className="px-2 pb-2">
          ...
          >
            <span className="text-xs font-bold text-gray-300">{lang === 'es' ? 'ES' : 'EN'}</span>
            {sideOpen && <span className="text-xs text-gray-400">{lang === 'es' ? '→ EN' : '→ ES'}</span>}
          </button>
        </div>
        ...
        <header className="bg-white border-b border-gray-100 px-6 py-3 flex items-center justify-between flex-shrink-0">
          <div>
            <p className="font-semibold text-gray-800 text-sm">
              {lang === 'es' ? NAV.find(n => n.id === page)?.labelEs : NAV.find(n => n.id === page)?.labelEn}
            </p>
            <p className="text-xs text-gray-400">Marquez Project Solutions LLC</p>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-xs text-gray-400">{new Date().toLocaleDateString(lang === 'es' ? 'es' : 'en', { weekday:'long', day:'numeric', month:'long', year:'numeric' })}</span>
