...
function Layout() {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-gray-50">
      <aside className={`flex flex-col bg-gray-900 transition-all duration-200 ${sideOpen ? 'w-56' : 'w-14'} flex-shrink-0`}>
        <div className="flex items-center gap-3 px-4 py-5 border-b border-gray-800">
          ...
          {sideOpen && (
            <div>
              ...
              <p className="text-gray-500 text-xs mt-0.5">ERP {isEs ? 'Construcción' : 'Construction'}</p>
            </div>
          )}
        </div>
        ...
        <div className="px-2 pb-2">
          <button onClick={() => toggleLang()}
            ...
            <span className="text-xs font-bold text-gray-300">{isEs ? 'ES' : 'EN'}</span>
            {sideOpen && <span className="text-xs text-gray-400">{isEs ? '→ EN' : '→ ES'}</span>}
          </button>
        </div>
        ...
        <header className="bg-white border-b border-gray-100 px-6 py-3 flex items-center justify-between flex-shrink-0">
          <div>
            <p className="font-semibold text-gray-800 text-sm">
              {isEs ? NAV.find(n => n.id === page)?.labelEs : NAV.find(n => n.id === page)?.labelEn}
            </p>
